...
export default function AdminOverviewPage() {
  ...
  const stats = [
    { label: "Total de clínicas",    value: loading ? "—" : String(totalClinics),      sub: `${activeClinics} ativas`,            icon: Building2,    color: "#1DB6A0" },
    { label: "Usuários cadastrados", value: loading ? "—" : String(totalMembers),       sub: "em todas as clínicas",              icon: Users,        color: "#6366f1" },
    { label: "WhatsApp conectados",  value: loading ? "—" : String(connectedWhatsApp),  sub: `de ${totalClinics} clínicas`,       icon: MessageSquare, color: "#22d3c0" },
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-z-dim mt-1">Gestão global da plataforma AgendaIAMed</p>
      </div>
        ...
        {stats.map((s, i) => {
          ...
          return (
            <motion.div
              ...
              style={{ background: "var(--surface-1)", border: "1px solid rgba(29,182,160,0.1)" }}
            >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          ...
          <a href="/admin/clinicas" className="text-xs text-[#1DB6A0] hover:text-[#22d3c0] transition-colors">
            Gerenciar →
          </a>
        </div>
        ...
        <div className="divide-y divide-border">
          {loading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-4">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <div className="h-2.5 w-28 rounded animate-pulse" style={{ background: "rgba(29,182,160,0.04)" }} />
                </div>
              </div>
            ))
          ) : clinics.length === 0 ? (
            <div className="px-5 py-10 text-center">
              ...
              <a href="/admin/clinicas" className="mt-2 text-xs text-[#1DB6A0] hover:text-[#22d3c0] block">
            ...
            clinics.slice(0, 8).map((clinic) => {
              ...
              return (
                <div key={clinic.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-[rgba(29,182,160,0.03)] transition-colors">
                  <div
